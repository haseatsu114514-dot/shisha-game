...
const TOURNAMENT_SCENE_PATH := "res://scenes/tournament/ch2_tournament.tscn"
...
## ゲスト審査員プール（南雲＋チャコール博士は固定。ch2ゲスト = DJ SMOKE）
const GUEST_JUDGES := [
	{"id": "dj_smoke", "name": "DJ SMOKE", "flavors": ["energy_drink", "mint", "blueberry"]},
]
...
## 弾幕演出強化定数（ch2: リージョナル大会 — やや難化）
const MIND_GRAZE_DISTANCE := 35.0
const MIND_GRAZE_FOCUS_BONUS := 3
const MIND_GRAZE_TIME_REDUCTION := 0.15  ## グレイズ1回あたり耐久時間を短縮する秒数
const MIND_FOCUS_SPEED_MULT := 0.42
const MIND_FOCUS_HITBOX_MULT := 0.4
const MIND_COMBO_DECAY_TIME := 2.5
const MIND_TRAIL_LENGTH := 6
const MIND_PATTERN_SPREAD_COUNT := 6
const MIND_PATTERN_BURST_COUNT := 10
const MIND_BASE_BULLET_SPEED_BONUS := 25.0  ## 章補正: 弾速加算
const MIND_SPAWN_INTERVAL_MULT := 0.9  ## 章補正: スポーン間隔倍率（低い=難しい）
const MIND_PHASE_SPEED_SCALING := 95.0  ## 後半ほど速くなる係数
...
func _prepare_run() -> void:
	...
	_easy_mode = bool(EventFlags.get_value("ch2_tournament_easy_mode", false))
...
func _on_mind_barrage_tick() -> void:
	...
	if _mind_screen_shake > 0.0:
		...
		if _mind_arena_layer != null and is_instance_valid(_mind_arena_layer):
			_mind_arena_layer.position = Vector2(
	...
	if _mind_spawn_cooldown <= 0.0:
		...
		if _mind_arena_layer != null and is_instance_valid(_mind_arena_layer):
			_mind_arena_layer.color = Color("181425", 0.95).lerp(Color("2a1520", 0.95), progress)
...
func _pick_barrage_category(progress: float) -> Dictionary:
	if progress < 0.35:
		return {"pool": MIND_WORDS_ANXIETY, "color": Color("8b9bb4", 0.9), "phase": "― 不安が湧き上がる ―", "pattern": "aimed"}
	elif progress < 0.7:
		var pattern = "aimed" if randf() < 0.6 else "spread"
		if randf() < 0.6:
			return {"pool": MIND_WORDS_AUDIENCE, "color": Color("feae34", 0.85), "phase": "― 会場の声が聞こえる ―", "pattern": pattern}
		else:
			return {"pool": MIND_WORDS_ANXIETY, "color": Color("8b9bb4", 0.9), "phase": "― 会場の声が聞こえる ―", "pattern": pattern}
	else:
		...
		var roll = randf()
		if roll < 0.4:
			return {"pool": MIND_WORDS_RIVAL, "color": Color("e43b44", 0.9), "phase": "― 心が折れそうだ ―", "pattern": pattern}
		elif roll < 0.7:
			return {"pool": MIND_WORDS_AUDIENCE, "color": Color("feae34", 0.85), "phase": "― 心が折れそうだ ―", "pattern": pattern}
		else:
			return {"pool": MIND_WORDS_ANXIETY, "color": Color("e43b44", 0.9), "phase": "― 心が折れそうだ ―", "pattern": pattern}
...
func _spawn_pattern_aimed(word_pool: Array, word_color: Color, progress: float, arena_size: Vector2) -> void:
	...
	_register_bullet(bullet, spawn, direction * _calc_bullet_speed(progress), size)
...
func _spawn_pattern_spread(word_pool: Array, word_color: Color, progress: float, arena_size: Vector2) -> void:
	...
	for i in range(count):
		...
		var size = _get_bullet_size(bullet, phrase)
		var spawn = _get_edge_spawn(side, arena_size, size)
		var base_dir = (_mind_player_pos - spawn).normalized()
		...
		_register_bullet(bullet, spawn, base_dir.rotated(angle_offset) * _calc_bullet_speed(progress) * randf_range(0.85, 1.1), size)
...
func _spawn_pattern_burst(word_pool: Array, word_color: Color, progress: float, arena_size: Vector2) -> void:
	...
	for i in range(count):
		...
		_register_bullet(bullet, center, Vector2(cos(angle), sin(angle)) * _calc_bullet_speed(progress) * randf_range(0.7, 1.0), size)
...
func _spawn_pattern_cross(word_pool: Array, word_color: Color, progress: float, arena_size: Vector2) -> void:
	for side in range(4):
		...
		if dir.length_squared() < 0.001:
			dir = Vector2.DOWN
		_register_bullet(bullet, spawn, dir * _calc_bullet_speed(progress) * 0.9, size)
...
func _get_edge_spawn(side: int, arena_size: Vector2, bullet_size: Vector2) -> Vector2:
	match side:
		0: return Vector2(randf_range(0.0, arena_size.x), -bullet_size.y * 0.5 - 4.0)
		1: return Vector2(arena_size.x + bullet_size.x * 0.5 + 4.0, randf_range(0.0, arena_size.y))
		2: return Vector2(randf_range(0.0, arena_size.x), arena_size.y + bullet_size.y * 0.5 + 4.0)
		_: return Vector2(-bullet_size.x * 0.5 - 4.0, randf_range(0.0, arena_size.y))
...
func _finish_mind_barrage_step() -> void:
	...
	_show_step_result_and_next(
		"%s\n被弾 %d / 回避 %d / GRAZE %d / MAX COMBO %d\n専門 %+d / 一般 %+d\n吸い出し速度補正: %s" % [
			result_text, hit_count, dodged, graze_count, max_combo,
			int(round(delta_spec)), int(round(delta_aud)), _mind_pull_adjust_text(),
		],
		_show_pull_step
	)
...
func _evaluate_mind_barrage_result() -> Dictionary:
	if _mind_lives_remaining <= 0:
		return {
			...
			"spec": -14.0, "aud": -5.0, "zone": -0.05, "heat_shift": 2,
			"pull_speed_adjust": 0.45, "force_worst_pull_speed": true,
		}
	...
	if resilience >= 0.86:
		return {"text": "表情が落ち着いた。冷静さを取り戻した。", "spec": 15.0, "aud": 6.0, "zone": 0.10, "heat_shift": -1, "pull_speed_adjust": -0.18, "force_worst_pull_speed": false}
	if resilience >= 0.68:
		return {"text": "揺れを抑えて、レシピに意識を戻した。", "spec": 8.0, "aud": 3.0, "zone": 0.05, "heat_shift": 0, "pull_speed_adjust": -0.10, "force_worst_pull_speed": false}
	if resilience >= 0.45:
		return {"text": "迷いは残るが、ギリギリ持ちこたえた。", "spec": 1.0, "aud": 0.0, "zone": 0.0, "heat_shift": 0, "pull_speed_adjust": 0.06, "force_worst_pull_speed": false}

	var panic_penalty = 3.0 if _mind_hits >= int(round(_mind_duration_total * 0.7)) else 0.0
	return {"text": "他人の価値観に呑まれ、心がブレた。", "spec": -9.0 - panic_penalty, "aud": -3.0, "zone": -0.03, "heat_shift": 1, "pull_speed_adjust": 0.14, "force_worst_pull_speed": false}


func _mind_pull_hint() -> String:
	if _mind_force_worst_pull_speed: return "最悪速度"
	if _mind_pull_speed_adjust <= -0.14: return "かなり遅い"
	if _mind_pull_speed_adjust <= -0.06: return "やや遅い"
	if _mind_pull_speed_adjust >= 0.10: return "速い"
	if _mind_pull_speed_adjust >= 0.04: return "やや速い"
	return "標準"
...
func _mind_pull_adjust_text() -> String:
	...
	var trend = "遅くなる"
	if _mind_pull_speed_adjust > 0.0: trend = "速くなる"
	elif abs(_mind_pull_speed_adjust) < 0.001: trend = "変化なし"
...
func _stop_mind_barrage() -> void:
	...
	for trail_node in _mind_trail_nodes:
		if trail_node != null and is_instance_valid(trail_node): trail_node.queue_free()
	_mind_trail_nodes.clear()
	for particle in _mind_particles:
		var pnode = particle.get("node")
		if pnode != null and is_instance_valid(pnode): pnode.queue_free()
	_mind_particles.clear()
	if _mind_player_node != null and is_instance_valid(_mind_player_node): _mind_player_node.queue_free()
	_mind_player_node = null
	if _mind_player_draw_node != null and is_instance_valid(_mind_player_draw_node): _mind_player_draw_node.queue_free()
...
func _mind_hit_flash() -> void:
	if _mind_arena_layer == null or not is_instance_valid(_mind_arena_layer): return
...
func _update_mind_trail() -> void:
	...
	while _mind_trail_nodes.size() > MIND_TRAIL_LENGTH * 3:
		...
		if old_node != null and is_instance_valid(old_node): old_node.queue_free()
...
func _spawn_hit_particles(center: Vector2, count: int, color: Color) -> void:
	...
	for i in range(count):
		...
		_mind_particles.append({"node": dot, "pos": center, "vel": Vector2(cos(angle), sin(angle)) * speed, "life": life, "max_life": life})
...
func _update_mind_combo_display() -> void:
	if _mind_combo_label != null and is_instance_valid(_mind_combo_label):
		if _mind_combo >= 3:
			...
			if _mind_combo >= 15: _mind_combo_label.add_theme_color_override("font_color", Color("e43b44"))
			elif _mind_combo >= 8: _mind_combo_label.add_theme_color_override("font_color", Color("feae34"))
			else: _mind_combo_label.add_theme_color_override("font_color", Color("00e5ff"))
		else:
			_mind_combo_label.text = ""
	if _mind_graze_label != null and is_instance_valid(_mind_graze_label):
		_mind_graze_label.text = "GRAZE %d" % _mind_graze_count if _mind_graze_count > 0 else ""
...
func _mind_phase_transition_effect() -> void:
	...
	var flash = ColorRect.new()
	var progress = clampf(_mind_elapsed / maxf(_mind_duration_total, 1.0), 0.0, 1.0)
	flash.color = Color("feae34", 0.3) if progress < 0.5 else Color("e43b44", 0.35)
	flash.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	flash.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_mind_arena_layer.add_child(flash)
	var tween = create_tween()
...
func _finalize_and_show_result() -> void:
	...
	if _player_rank == 1:
		EventFlags.set_value("ch2_tournament_loss_count", 0)
	else:
		_pending_reward = 0
		var losses = int(EventFlags.get_value("ch2_tournament_loss_count", 0)) + 1
		EventFlags.set_value("ch2_tournament_loss_count", losses)
	...
	if _player_rank == 1:
		...
		var losses = int(EventFlags.get_value("ch2_tournament_loss_count", 0))
...
func _apply_result_and_continue() -> void:
	...
	if _player_rank == 1:
		...
		EventFlags.set_value("ch2_tournament_easy_mode", false)
	else:
		PlayerData.add_stat("insight", 1)
		GameManager.log_stat_change("insight", 1)

	EventFlags.set_flag("ch2_tournament_completed", true)
	EventFlags.set_value("ch2_tournament_rank", _player_rank)
...
func _enable_easy_mode_and_retry() -> void:
	EventFlags.set_value("ch2_tournament_easy_mode", true)
	get_tree().change_scene_to_file(TOURNAMENT_SCENE_PATH)
