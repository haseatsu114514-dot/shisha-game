...
## ゲスト審査員プール（南雲＋チャコール博士は固定。ch4ゲスト = ダ・シルヴァ太陽）
const GUEST_JUDGES := [
	{"id": "da_silva", "name": "ダ・シルヴァ太陽", "flavors": ["tropical_fruit", "mango", "pineapple"]},
]
...
## 弾幕演出強化定数（ch4: 世界大会 — 最高難易度）
const MIND_GRAZE_DISTANCE := 28.0
const MIND_GRAZE_FOCUS_BONUS := 3
const MIND_GRAZE_TIME_REDUCTION := 0.10  ## グレイズ1回あたり耐久時間を短縮する秒数
const MIND_FOCUS_SPEED_MULT := 0.42
const MIND_FOCUS_HITBOX_MULT := 0.4
const MIND_COMBO_DECAY_TIME := 2.5
const MIND_TRAIL_LENGTH := 6
const MIND_PATTERN_SPREAD_COUNT := 8
const MIND_PATTERN_BURST_COUNT := 16
const MIND_BASE_BULLET_SPEED_BONUS := 80.0  ## 章補正: 弾速加算
const MIND_SPAWN_INTERVAL_MULT := 0.7  ## 章補正: スポーン間隔倍率（低い=難しい）
const MIND_PHASE_SPEED_SCALING := 130.0  ## 後半ほど速くなる係数
...
func _prepare_run() -> void:
	...
	_easy_mode = bool(EventFlags.get_value("ch4_tournament_easy_mode", false))
...
func _start_mind_barrage_step() -> void:
	...
	face_label.text = ""
	face_label.add_theme_font_size_override("font_size", 11)
	face_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	face_label.autowrap_mode = TextServer.AUTOWRAP_WORD
	...
	phase_hint.add_theme_color_override("font_color", Color("8b9bb4", 0.55))
...
func _mind_update_face() -> void:
	if _mind_arena_layer == null or not is_instance_valid(_mind_arena_layer):
		return
	var parent = _mind_arena_layer.get_parent()
	if parent == null:
		return
	var hbox = parent.get_parent()
	if hbox == null:
		return
	var face_panel = hbox.get_child(0) if hbox.get_child_count() > 0 else null
	if face_panel == null:
		return
	var face_node = face_panel.get_node_or_null("MindFaceRect") as TextureRect
	var label_node = face_panel.get_node_or_null("MindFaceLabel") as Label
	if face_node == null:
		return
	var life_ratio = float(_mind_lives_remaining) / float(maxi(_mind_lives_max, 1))
	...
	if life_ratio <= 0.0:
		chosen_face = "sad"
		chosen_text = "（もう…ダメだ…）"
	elif life_ratio <= 0.34:
		chosen_face = "surprise"
		chosen_text = "（やばい…集中しろ…！）"
	elif life_ratio <= 0.67:
		chosen_face = "serious"
		chosen_text = "（まだいける…！）"
	else:
		var progress = clampf(_mind_elapsed / maxf(_mind_duration_total, 1.0), 0.0, 1.0)
...
func _finalize_and_show_result() -> void:
	...
	if _player_rank == 1:
		EventFlags.set_value("ch4_tournament_loss_count", 0)
	else:
		_pending_reward = 0
		var losses = int(EventFlags.get_value("ch4_tournament_loss_count", 0)) + 1
		EventFlags.set_value("ch4_tournament_loss_count", losses)
	...
	if _player_rank == 1:
		...
		var losses = int(EventFlags.get_value("ch4_tournament_loss_count", 0))
...
func _apply_result_and_continue() -> void:
	if _player_rank == 1: # Assuming _player_rank is the correct variable for current rank
		...
		EventFlags.set_value("ch4_tournament_easy_mode", false)
		PlayerData.add_stat("insight", 1)
		GameManager.log_stat_change("insight", 1)

	EventFlags.set_flag("ch4_tournament_completed", true)
	EventFlags.set_value("ch4_tournament_rank", _player_rank)
...
func _enable_easy_mode_and_retry() -> void:
	EventFlags.set_value("ch4_tournament_easy_mode", true)
	get_tree().change_scene_to_file(TOURNAMENT_SCENE_PATH)
