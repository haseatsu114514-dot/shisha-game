...
const TOURNAMENT_SCENE_PATH := "res://scenes/tournament/ch3_tournament.tscn"
...
## ゲスト審査員プール（南雲＋チャコール博士は固定。ch3ゲスト = エミル）
const GUEST_JUDGES := [
	{"id": "emil", "name": "エミル・ユルマズ", "flavors": ["pistachio", "double_apple", "mint"]},
]
...
## 弾幕演出強化定数（ch3: 全国大会 — 高難易度）
const MIND_GRAZE_DISTANCE := 32.0
const MIND_GRAZE_FOCUS_BONUS := 3
const MIND_GRAZE_TIME_REDUCTION := 0.12  ## グレイズ1回あたり耐久時間を短縮する秒数
const MIND_FOCUS_SPEED_MULT := 0.42
const MIND_FOCUS_HITBOX_MULT := 0.4
const MIND_COMBO_DECAY_TIME := 2.5
const MIND_TRAIL_LENGTH := 6
const MIND_PATTERN_SPREAD_COUNT := 7
const MIND_PATTERN_BURST_COUNT := 12
const MIND_BASE_BULLET_SPEED_BONUS := 50.0  ## 章補正: 弾速加算
const MIND_SPAWN_INTERVAL_MULT := 0.8  ## 章補正: スポーン間隔倍率（低い=難しい）
const MIND_PHASE_SPEED_SCALING := 110.0  ## 後半ほど速くなる係数
