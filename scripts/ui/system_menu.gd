...
const CHARACTERS = [
	...
	{"id": "mashiro", "name": "ましろ", "condition": "ch3_forced_mashiro_encounter_done"},
]
...
func _populate_affinity() -> void:
	...
	for char_data in CHARACTERS:
		...
		if (
			char_id in ["naru", "adam", "minto", "tsumugi", "ageha", "mashiro"]
			and not EventFlags.get_flag("known_name_" + char_id)
		):
			display_name = "？？？"
