...
const REEL = (() => {
  ...
  // 回せる回数が少ない（1章 ≒ 28回転）前提で逆算:
  //   リプレイ 1/7.3（実機オマージュ）／チェリー 1/9.1（うち約1/8がボーナス重複）／
  //   ベル 1/83（レア小役）／中段チェリー等 1/167（BIG確定プレミア）／
  //   ペカ合算 ≒ 1/13（章2回ちょい）／フリーズは別枠制御（下記）
  const ROLES = [
    { id: "miss",   weight: 677 },
    { id: "replay", weight: 137 },
    { id: "cherry", weight: 110 },
    { id: "bell",   weight: 12 },
    { id: "rare",   weight: 6 },   // 中段チェリー/単独パッキー → BIG確定（プレミア告知）
    { id: "reg",    weight: 26 },
    { id: "big",    weight: 28 },
    { id: "freeze", weight: 4 },   // 名目値。実際は FREEZE_CONTROL が管理する
  ];
  const PEKA = ["reg", "big", "freeze", "rare"]; // ランプが光る役（チェリー重複は別途）
  // チェリー重複: チェリーのうち約1/8で同時当選（重複ペカ）
  const CHERRY_OVERLAP = { chance: 0.125, bigShare: 0.45 };
  // フリーズ制御:「完全確率に見せかけて管理」
