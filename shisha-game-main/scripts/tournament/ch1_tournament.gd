...
func _create_stage_card(title_text: String, subtitle_text: String = "", min_height: float = 0.0) -> Dictionary:
	...
	accent_bar.color = Color("feae34", 0.9)
	accent_bar.size_flags_vertical = Control.SIZE_EXPAND_FILL
	title_row.add_child(accent_bar)
...
func _show_stage_briefing(step_num: int, title: String, body: String, lines: Array, tutorial_lines: Array[String], start_label: String, start_callback: Callable) -> void:
	...
	if _show_tutorial and not tutorial_lines.is_empty():
		...
		tutorial_style.border_width_top = 1
		...
		tutorial_style.content_margin_left = 20
		tutorial_style.content_margin_top = 18
		tutorial_style.content_margin_right = 20
		tutorial_style.content_margin_bottom = 18
		tutorial_card["panel"].add_theme_stylebox_override("panel", tutorial_style)
		...
		for tutorial_line in tutorial_lines:
			...
			row.add_theme_color_override("font_color", Color("b8dcff"))
...
func _register_bullet(bullet: Label, spawn: Vector2, vel: Vector2, size: Vector2) -> void:
	var data := {
		...
		"size": size,
		"grazed": false,
		"base_modulate": bullet.modulate,  # グレイズグロー復元用
		"graze_glow": 0.0,
	}
