...
func _show_affinity_notification(char_id: String, _delta: int) -> void:
	...
	ac.add_point(Vector2(0.12, 1))
	ac.add_point(Vector2(0.65, 0.8))
	ac.add_point(Vector2(1, 0))
	alpha_curve.curve = ac
	mat.alpha_curve = alpha_curve

	# Pink heart color matching character accent
	var heart_color = notif_color.lerp(Color(1.0, 0.5, 0.7), 0.5)
	heart_color.a = 0.92
	mat.color = heart_color
	particles.process_material = mat
	particles.amount = 18
	particles.lifetime = 1.2
	particles.one_shot = true
	particles.position = NOTIFICATION_BASE_POSITION + Vector2(NOTIFICATION_CARD_SIZE.x * 0.4, 54)

	# Heart-shaped texture (procedural)
	var himg = Image.create(12, 12, false, Image.FORMAT_RGBA8)
	var hcenter = Vector2(6, 6)
	for hx in range(12):
		for hy in range(12):
			var nx = (float(hx) / 12.0) * 2.0 - 1.0
			var ny = (float(hy) / 12.0) * 2.0 - 0.8
			var heart_val = (nx * nx + ny * ny - 1.0)
			var heart_val2 = (nx * nx + (ny - absf(nx) * 0.5) * (ny - absf(nx) * 0.5) - 1.0)
			if heart_val2 < 0.2:
				var alpha = clampf(1.0 - (heart_val2 + 0.2) / 0.5, 0, 1)
				himg.set_pixel(hx, hy, Color(1, 1, 1, alpha * 0.9))
			else:
				himg.set_pixel(hx, hy, Color(0, 0, 0, 0))
	particles.texture = ImageTexture.create_from_image(himg)
	...
	fade_tween.tween_property(card, "position:y", NOTIFICATION_BASE_POSITION.y - 32.0, 0.38)
