...
| `minto` | 翠川 栞 | ライバル ch1（自称20歳・実年齢29歳） |
...
| `takiguchi` | 焚口 ショウ | 大会実況MC（人間） |
| `packii` | パッキー | 大会マスコットMC（モノクマ系。笑い声:「ぷぷぷっ！」） |
| `emil` | エミル | 審査員（元トルコ職人） |
...
全文読み込みはトークンを多く消費します。Gemini（あなた自身）のファイル閲覧ツール（`view_file`、`grep_search`等）を駆使し、必要な箇所のみ読んでください。

| ファイル | 行数 | 推奨アクセス方法 |
|---|---|---|
| `scripts/tournament/ch1_tournament.gd` | 4,261行 | 該当関数名で Grep (`grep_search`) してから部分読み取り |
| `data/dialogue/ch1_main.json` | 926行 | `dialogue_id` で Grep して前後だけ読む |
| `data/dialogue/ch1_tournament.json` | 838行 | 同上 |
| `data/characters.json` | 656行 | `jq`コマンド、または `grep_search` で1件抽出 |
