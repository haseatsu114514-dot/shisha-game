...
**ステータス: 実装待ちパッケージ（本編未接続）**。
コードは完成・テスト済みで、オーナーのGOが出たらこの手順書どおりに接続すれば動く。
他機能の開発と衝突しないよう、既存ファイル（game.js / sfx.js / style.css / index.html /
build_standalone.py）には一切手を入れていない。
...
| 図柄 | 役 | 確率（基本） | 恩恵（対象ステへ） |
|---|---|---|---|
| ☁ 煙（ブランク） | ハズレ | 67.7% | **+1（サイレント）**＝最低保証 |
| 💧 リプレイ（水の青） | 再遊技 | **1/7.3**（実機オマージュ） | +1（サイレント）＋即もう1回転（連鎖上限4） |
| 🍒 チェリー（角） | 小役 | 1/9.1 | +2「少し上がった」 |
| └ チェリー**重複** | 同時当選 | チェリーの**約1/8** | 小役の後にひと呼吸おいてペカッ（BIG 45% / REG 55%） |
| 🍍 パイナップルのベル | レア小役 | 1/83 | +3「上がった」 |
| BAR | REG | 1/38.5 | +4「上がった」 |
| 赤7 | BIG | 1/35.7 | +6「大きく上がった」＋ジャグ連ゾーン |
| 🍒中段 / ぷ単独（パッキー柄） | **プレミア** | 1/167 | BIG確定（+6）＋虹ランプ告知 |
| （フリーズ） | ロングフリーズ | 制御役（§5） | +8「大きく上がった」＋ゾーン＋殿堂入り記録 |

1章=14日×2行動=**約28回転**しか回せない前提で逆算した数字（オーナー指示:「回数に限りがあるので小役確率は慎重に」）。
テスト実測の1章あたり期待値：

```
ハズレ18.5 ／ リプレイ3.9 ／ チェリー3.1（うち重複0.4） ／ ベル0.33 ／
REG 0.9 ／ BIG 1.05 ／ プレミア0.17 ／ ペカ合算 約2.5回（救済込み実測 1/11.2）
リール由来の総ステ上昇 ≒ 42pt/章（※リッチなら EFFECTS の exp を絞る。§9）
```
...
- 現状 exp 合計≒42pt/章はやや富裕。絞るなら `EFFECTS` の exp（特に miss/replay の+1は
