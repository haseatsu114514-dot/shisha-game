...
リールは3本×16コマ（`STRIPS`、**ジャグラー配列を参考**）。**有効ラインは中段のみ＝実機ジャグラー準拠で
「下段揃い」は役にしない**。同じ図柄を縦に隣接させない配列なので、中段で揃っても上下段は揃わない
（テスト `reel.mjs` が縦連続ゼロと下段非揃いを検証）。
ボーナスは**ハズレ目で止まってランプが教える**（ジャグラー流）。揃えるのは精算演出の中
（全画面スロット画面で 7／BAR を中段に揃え直す）。図柄: 赤7=BIG・BAR=REG・
