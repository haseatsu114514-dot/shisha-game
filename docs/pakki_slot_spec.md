# MOKU!MOKU!パッキー（日常リール）— 仕様書 兼 組み込み手順書
...
- C.STATION公式マスコットアプリという体ではじめのスマホに入っている。
  **1行動=1回転**：日常パートで行動を消費するたび、全画面のスロット画面（ジャグラー風筐体）が出て1回転
...
リールは3本×16コマ（`STRIPS`、**ジャグラー配列を参考**）。**有効ラインは「中央＋右下がり＋右上がりの斜め」
の3ライン**（実機ジャグラーは上下段も含む5ラインだが、オーナー方針で横の上段・下段は役にしない）。
同じ図柄を縦に隣接させない配列なので、横一列（上段/下段）に同図柄3つが並ぶことはない
（テスト `reel.mjs` が縦連続ゼロ・横揃い非成立・各役が指定ライン上に揃うことを検証）。
当たりは中央/斜めのいずれかにランダムで揃い、ペイラインが点滅する。
ボーナスは**ハズレ目で止まってランプが教える**（ジャグラー流）。揃えるのは精算演出の中
（全画面スロット画面で 7／BAR を当該ラインに揃え直す）。図柄: 赤7=BIG・BAR=REG・
**ベル=ジャグラーのぶどう枠（常連小役）**・角チェリー・リプレイ(水の青)・煙ブランク・パッキー柄(ピエロ)。

| 図柄 | 役 | 確率（基本） | 恩恵（対象ステへ） |
|---|---|---|---|
| 🔔 ベル（金の鐘＝ぶどう枠） | 常連小役 | **1/6.7** | +2「少し上がった」（コツコツ） |
| 💧 リプレイ（水の青） | 再遊技 | **1/7.3**（実機オマージュ） | +1（サイレント）＋即もう1回転（連鎖上限4） |
| ☁ 煙（ブランク） | ハズレ | 60.4% | **+1（サイレント）**＝最低保証 |
| 🍒 チェリー（角） | 小役 | 1/25 | +2「少し上がった」 |
| └ チェリー**重複** | 同時当選 | チェリーの**約1/16** | 小役の後にひと呼吸おいてペカッ（BIG 45% / REG 55%） |
| BAR | REG（バケ） | **1/31** | +4「上がった」 |
| 赤7 | BIG | 1/36 | +6「大きく上がった」＋ジャグ連ゾーン |
| 🍒中段 / ぷ単独（パッキー柄） | **プレミア** | 1/200 | BIG確定（+6）＋虹ランプ告知 |
| （フリーズ） | ロングフリーズ | 制御役（§5） | +8「大きく上がった」＋ゾーン＋殿堂入り記録 |

1章=14日×2行動=**約28回転**しか回せない前提。2026-06-13 にジャグラー寄せの再構成:
**ぶどう枠をベルに置換して常連小役化**（1/6.7・コツコツ +2）／チェリーは角チェリー化して 1/25 に／
**バケ(REG)を少し増量**（オーナー指示）。名目ペカ合算は **約1/14**。テスト実測の1章あたり期待値：

```
ベル4.2 ／ リプレイ3.9 ／ ハズレ16.6 ／ チェリー1.1（うち重複ごく稀） ／
REG 1.08 ／ BIG 1.03 ／ プレミア0.13 ／ ペカ合算 約2.3回（長期実測 1/12.2）
リール由来の総ステ上昇 ≒ 43pt/章（※リッチなら EFFECTS の exp を絞る。§9）
```
