...
> ⚠️ **用語に関する注記（2026-06-13 / master_spec #17）**: 本書に「弾丸」「論破」「実証リング」等の
> 表現が残っている場合、それらは**すべて旧称・廃止**。正式用語は **「コンセプト」**（制作前に選ぶ方針）と
> **「アピールポイント」**（制作実績から自動生成される持ちネタ）、動詞は **「ぶつける」**。
> 反論ワードも「嘘だ！」→「香ってない！」「ズレてる！」に統一する。詳細は `docs/master_spec.md` 第1部#17・第2部。
