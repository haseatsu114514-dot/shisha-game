...
- **ch4=和風一本（ダブルアップル要素なし）／ch5=ダブルアップル単体に戻す**。
  ch3事件の贖罪とアダムへのリスペクトの回収は**ch5のダブルアップル単体勝負**へ移動
...
3. **あげはの消えた背中の回収** — `ch4_ageha_dubai.json`（彼女ルート限定）。ch2県大会で敗れた直後『……ごめん。理由は、聞かないで』の裏側（はじめの空っぽな煙に負けたことが直感への自信を折った）を一年越しに本人が明かす。story doc ch2/ch4 両方に正典化
...
5. **みんとの嘘の鏡像**（`ch3_minto_small_lies`・ダブルアップル事件後のLIME通話）—「嘘ってさ、つき始めは小さいんだよ」。彼女の告白シーンとch5贖罪の縦糸を接続
...
- **蒸らし＝雑念弾幕（アンダーテール風の回避）**（`runSteamDodge`・2026-06-14 オーナー指定で「覚悟＋締め」から差し戻し。現デザインのタイマー＋鼓動の枠は維持）: 0/3/5/8/10分から選択（長いほど被弾に晒される時間も長い）。心(♥)をドラッグ/矢印で動かして雑念弾を躱す。被弾が少ないほど craft＋（採点は `tt.steamHits` を読む。0=締め相当の最高）。0分（吸いながら立ち上げる）は神崎竜二との交友で解放＝弾幕スキップ。テストは `window.__steamDebug().end()` で即終了（無操作＝被弾0）。DODGE_WORDS/DODGE_WORDS_CH2 が弾の文言。
...
3. **日常/経済/UX**: #41 体力を蓄積制(寝る=全回復/恋人=好感度で一定) ／ #44 フレーバー持参制(不足→スミ案内の強制購入/無一文→ダブルアップル1回分) ／
...
- **#43残: なるの敗者復活戦バックストーリー**: ルール側は廃止反映済(2位以下=GAME OVER)。但し ch4_main.json/characters.json の
  なるの「全国で負け→敗者復活を勝ち抜いた」設定が残る＝廃止と矛盾。なるの背景を別ルート(連敗からの再挑戦等)に書き換えるか要判断。
...
- **新規シナリオデータ（ch2/ch3用・エンジン未接続）**:
  ...
  - `data/dialogue/ch3_double_apple.json` — mukai到着・**ダブルアップル事件**
    （グレープをダブルアップルと言われて同調→嘘がバレる。シトラスミント・シナモンは旧設定・2026-07-20変更）・毎日の一杯・味覚回復
