...
- **ダブルアップル（DA）の味の正史（2026-07-20確定・絶対にブラさない）**: 林檎そのものの味ではなく、**アニス＝スパイスの香りで林檎を再現した、シーシャにしかない独特な味**。独特だからこそシーシャ好きなら一口でそれと分かる。❌「林檎飴の甘さ」「果汁感」等、果実の林檎味としての描写は禁止。ch3のDA事件でましろが出すのは**シナモン**（同じスパイス系だから壊れた舌だけが取り違える。❌シトラスミントは旧設定）。
