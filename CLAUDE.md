...
- **各章の大会は1大会1試合のみ（2026-07-20確定）**: 予選・準決勝などの多段戦は存在しない。章の最終日（DAY14）に4人一斉の一発勝負を行う。出場4人は予選ライン・推薦枠など**大会の外**で決まっている（❌ch2の「予選→準決勝→決勝」3試合制は旧実装）。**例外はch5のみ**（SHISHA-9000との1vs1・2本勝負）。ch2の対戦相手は**あげは・神崎竜二・零-REI-**、ch3は**ナンディ・スティーブ・ヴォルク**、ch4は**なる・王煙楼・シェイク**
- **審査員持ち点投入制**: 各審査員は持ち点10を好きなタイミングで投入（戻せない）。南雲は最後まで温存し「もう一口吸いたい一台」に一括投入する流儀。ch1は craft 基準未達だと南雲票が動かず普通に敗北、基準超えで最終逆転（実装済み）
- **章ごとの課題フレーバー（レギュレーション）**: ch1=ミント2g以上／ch2=ストロベリー（県特産・高難度）／ch3=自由（ましろと作戦会議3択）／ch4=自由＋日本アピール（スミさんと作戦会議3択）／ch4=和風（和素材で日本をアピール。ダブルアップル要素は無し）／ch5=ダブルアップル単体（配合自由度ゼロの技術勝負・ch3事件の贖罪の最終回収）。詳細は brand/story_and_structure.md の表
- C.STATIONが正式名称（❌「シーシャ・ステーション」「シーシャステーション」は旧表記）。**読みは「シーステーション」＝店舗ブランド名**（南雲が会長を務めるチェーン・2026-07-19確定）
...
- **ダブルアップルの味の正史（2026-07-20確定・絶対にブラさない）**: 林檎そのものの味ではなく、**アニスとリコリス──二つのスパイスの香りで林檎を再現した、シーシャにしかない独特な味**。独特だからこそシーシャ好きなら一口でそれと分かる。❌「林檎飴の甘さ」「果汁感」等、果実の林檎味としての描写は禁止。❌略記「DA」も禁止（必ず「ダブルアップル」と書く）。ch3のダブルアップル事件でましろが出すのは**グレープ**（アニスとリコリスの気配が一粒もない果実味。健康な舌なら間違えようがない二つの取り違えが、味覚喪失の動かぬ証拠になる。❌シトラスミント・シナモンは旧設定）。
...
| `rei` | 田中 健太 | ライバル ch2（V系。ch1バイトに正体を伏せてカメオ来店済み。県大会で敗退し、ch3は東京のシーンで本格登場＝大会には出ない・2026-07-20確定） |
| `dr_kemuri` | チャコール博士 / 炭場 創 | ライバル ch2・裏ボス戦の仕掛け人 ch5（❌「ドクター・ケムリ」は旧名） |
| `mashiro` | 水瀬 ましろ | ヒロイン ch3（別名:シガーマン。❌「真白 ましろ」は旧表記・2026-07-18確定） |
| `mukai_master` | ムカイさん / 向井 | ch3 溜まり場「mukai」店長 |
| `tetsuko` | テツコ | mukai 裏口の猫 |
| `nandi` | ナンディ・カルダモン | ライバル ch3 |
| `steve` | スティーブ・デイビス | ライバル ch3 |
| `volk` | ヴォルク・イヴァノフ | ライバル ch3（ch2は店「鉄の煙」の先行登場のみ・県大会には出ない。2026-07-18確定） |
| `master_hookah` | 王 煙楼 | ライバル ch4（世界大会の大ボス格。決勝4人の一角） |
...
- **ch4 の優勝の一台は和風**（和素材で日本をアピール。ダブルアップル要素は入れない）。**ch3ダブルアップル事件の贖罪はch5のダブルアップル単体勝負で回収**（アダムへのリスペクトも同所）
- **ch4 決勝は4人一斉**（はじめ・なる・王煙楼・シェイク）。山場はなる→王煙楼→シェイクの順。はじめの決勝の一台は「和の一台」ひとつ（2026-07-18確定）
- **第5章の章題は「機械仕掛けの神（Deus Ex Machina）」**。「煙の向こう側」は章題にも使わない（2026-07-18確定）
