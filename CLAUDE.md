...
- **審査員持ち点投入制**: 各審査員は持ち点10を好きなタイミングで投入（戻せない）。南雲は最後まで温存し「もう一口吸いたい一台」に一括投入する流儀。ch1は craft 基準未達だと南雲票が動かず普通に敗北、基準超えで最終逆転（実装済み）。敗者復活戦は余り点が原資
...
- **吸い出し**: 炭を置いて蒸らした後、提供前に何度か吸って温度を立ち上げる工程（最低2回）。吸い方で熱を上げ下げできる。
- **プレゼン工程は廃止（2026-06-12 オーナー決定）**: 大会は「作る → 蒸らす → 吸い出しで温度を作る → 提供」で完結し、審査員が吸って評価する。プレゼン描写・プレゼン工程をコードやシナリオに復活させないこと。
