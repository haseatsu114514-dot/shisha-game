...
- 上昇通知は**抽象表現のみ**（2026-07-05 4段階に改定・N20）:
  - 1〜2ポイント → 「少し上がった」
  - 3〜4ポイント → 「上がった」
  - 5〜7ポイント → 「かなり上がった」
  - 8ポイント以上 → 「大きく上がった」
- セリフ内の表記例: `……【技術】と【センス】が上がった。`
- **同じステータスへの複数回の加算は1つの通知バナーへ自動で合算される**（`web/js/game.js` の
  `gainStat()`→`statGainBatch`。短い時間窓に入った同一項目の増分を合計してから上の4段階で表示）。
  違うステータス（例: 技術と魅力が同時に伸びる）は従来どおりそれぞれ別バナーで良い。
  台詞側で直接「かなり上がった」と書いても`parseTextCue()`が対応する加算量を解釈する。
...
| `oneesan` | お姉さん | みんとの私服・素の姿用エイリアス（立ち絵は minto の `ura_*`。通常営業では正体を伏せるが、みんと訪問5回目の私服デート `ch1_minto_fifth` でch1内でも正体が判明する＝正史） |
...
| `suno-bgm-prompt` | 新規BGMのSuno AI用プロンプト作成と受け取り後の組み込み（キー命名・BGM_FILES・75秒制約） |
...
- **みんとの私服=「お姉さん」**(speaker `oneesan`)。通常営業では正体を伏せるが、みんと訪問イベントを進めれば **ch1内でも訪問5回目の私服デート（`ch1_minto_fifth`）で正体が判明する**（正史）。身分証確認の記憶が「自称20歳」の嘘の伏線
...
ステ上昇・好感度は `web/js/game.js` の `parseTextCue()` が**台詞テキストを正規表現で読んで**発火する（`/(技術|センス|根性|魅力|洞察|好感度)/` ＋「上がった／少し上がった／かなり上がった／大きく上がった」）。これは台詞を書き換えると報酬が黙って壊れる／重複する温床。

- 報酬を確実にしたいイベントは、地の文頼みにせず `type:"apply"` か metadata で明示的に付ける。
- 台詞で報酬を表すときは**規定の言い回しを厳守**: `……【技術】と【センス】が上がった。`（1〜2pt=少し上がった／3〜4pt=上がった／5〜7pt=かなり上がった／8pt〜=大きく上がった）。
