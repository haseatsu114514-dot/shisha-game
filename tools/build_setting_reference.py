...
CHAPTERS = [
    ...
    dict(no="第4章", title="継承の煙（レガシー・スモーク）", cup="HAZE: GRAND SMOKE（世界大会・ドバイ）", reg="自由＋日本アピール（和風一本・DA要素なし）",
         theme="全部背負って、最後に出す一台", img="cg_battle",
         imgcap="CG: 大会の激突（cg_ch1_tournament_battle）",
         body="""スミさんとの特訓を経てドバイへ。前夜にはヒロインたちが応援に駆けつける（登場条件は下の表）。現地では「謎のおじさん」（変装したシェイク）との強制イベント×3、チャドの店・太陽の店というミスディレクションを挟み、大会当日にすべての正体が明かされる。<br><br>転-1は<strong>はじめ vs なる</strong>——イチから勝ち直してきた男との、ゲーム最大の感動パート。転-2は絶対王者シェイクとの最終対決。はじめは<strong>和素材×tonariの日常の丁寧さの一台</strong>で世界一に。帰国後、スミさんが告白する: 15年前ドバイでシェイクを感動させた日本人は自分であり、折れた理由は「勝つための煙で、客の顔が見えなくなったから」——はじめのch2の鏡像。""",
         points=["南雲の「懐かしい味だ」・エミルの噂・シェイクの原体験、すべての縦糸がスミさんに収束する",
                 "打ち上げ（交際2人以上なら修羅場イベント）／スミさんとの最後の一杯",
                 "ラスボス: 王 煙楼（マスター・フーカ）、ゲスト審査員: ダ・シルヴァ太陽"]),
...
<section class="block" id="story">
  ...
  <ul class="rules">
    ...
    <li><strong>継承の対句</strong> — 「技は盗め。ただし、誰から盗んだかは忘れるな」（ch1スミ）→「盗むのと、借り物で着飾るのは違う」（ch2なる）→ DA単体勝負＝誰から始まった味かを忘れない（ch5）。</li>
